...
class ImportFileEmployeeAttendance implements ToModel
{
    ...
    public function model_bk_230368(array $row)
    {
    ...
    public function model_bk_230368_2(array $row)
    {
        ...
        DB::transaction(function () use ($row, $checkYear) {
            ...
            if ($row[15] && $row[15] != "") {
                ...
                if (count($cut) === 3) {
                    ...
                    // วันที่ไม่อยู่ในรูปแบบที่คาดหวัง เช่น ไม่มี / หรือ format ผิด
                    $attendanceDate = null;
                }
            } else {
                $attendanceDate = null;
    ...
    public function model(array $row)
    {
        ...
        try {
            foreach ($rows as $row) {
                if (!isset($row['EMPLOYEE_NO']) || empty($row['EMPLOYEE_NO'])) {
                    continue;
                }

                $employeeNo = sprintf("%06d", $row['EMPLOYEE_NO']);

                // แปลงวันที่
                $attendanceDate = null;
                if (!empty($row['DATE_JOINED'])) {
                    $cut = explode('/', $row['DATE_JOINED']);
                    if (count($cut) === 3) {
                        $cut1 = (int)$cut[2] - 543;
                        $attendanceDate = $cut1 . '-' . $cut[1] . '-' . $cut[0] . ' 00:00:00';
                    }
                ...
                AttendanceLog::create([
                    ...
                    'EMPLOYEE_LOCAL_NAME' => $row['EMPLOYEE_LOCAL_NAME'] ?? '',
                    ...
                    'attendance_sl'       => $row['SL'] ?? 0,
                    'attendance_pl'       => $row['PL'] ?? 0,
                    'attendance_late'     => $row['LATE'] ?? 0,
                    'attendance_abs'      => $row['ABS'] ?? 0,
                    'attendance_abt'      => $row['ABT'] ?? 0,
                    'attendance_cl'       => $row['CL'] ?? 0,
                    'attendance_ol'       => $row['OL'] ?? 0,
                    'attendance_sus'      => $row['SUS'] ?? 0,
                    'attendance_wwar'     => $row['WWAR'] ?? 0,
                    'attendance_vwar'     => $row['VWAR'] ?? 0,
                    'attendance_score'    => ($row['SL'] ?? 0) + ($row['PL'] ?? 0) + ($row['LATE'] ?? 0) + ($row['ABS'] ?? 0),
                    'not_up_salary'       => $row['พนักงานที่ไม่ได้ปรับค่าแรง'] ?? null,
                    'created_by'          => $userId,
                    'updated_by'          => 0,
                    'created_at'          => now(),
                    'updated_at'          => null,
                ...
                // Master tables
                Position::updateOrCreate(['position_code' => $row['POSITION_CODE'] ?? ''], ['POSITION_DESCRIPTION' => $row['POSITION_DESCRIPTION'] ?? '', 'created_by' => $userId]);
                Division::updateOrCreate(['division_code' => $row['DIVISION_CODE'] ?? ''], ['DIVISION_DESCRIPTION' => $row['DIVISION_DESCRIPTION'] ?? '', 'created_by' => $userId]);
                Department::updateOrCreate(['department_code' => $row['DEPARTMENT_CODE'] ?? ''], ['DEPARTMENT_DESCRIPTION' => $row['DEPARTMENT_DESCRIPTION'] ?? '', 'created_by' => $userId]);
                Section::updateOrCreate(['section_code' => $row['SECTION_CODE'] ?? ''], ['SECTION_DESCRIPTION' => $row['SECTION_DESCRIPTION'] ?? '', 'created_by' => $userId]);
                Grademaster::updateOrCreate(['grade_code' => $row['GRADE_CODE'] ?? ''], ['GRADE_DESCRIPTION' => $row['GRADE_DESCRIPTION'] ?? '', 'created_by' => $userId]);

                // EmployeeModel
                EmployeeModel::updateOrCreate(
                    ['orisoft_no' => $employeeNo],
                    [
                        'employee_local_name_en' => $row['EMPLOYEE_NAME'] ?? '',
                        'employee_local_name_th' => $row['EMPLOYEE_LOCAL_NAME'] ?? '',
                        'position_code'          => $row['POSITION_CODE'] ?? '',
                        'position_description'   => $row['POSITION_DESCRIPTION'] ?? '',
                        'division_code'          => $row['DIVISION_CODE'] ?? '',
                        'division_description'   => $row['DIVISION_DESCRIPTION'] ?? '',
                ...
                // Final Score
                $calcompliance = 10 - (($row['ABT'] ?? 0) + ($row['VWAR'] ?? 0 * 2) + ($row['WWAR'] ?? 0 * 5) + ($row['SUS'] ?? 0 * 10));
                $salaryType = (($row['grade_code'] ?? '') === 'L800') ? 'Daily' : 'Monthly';
                $salaryOld = ($salaryType === 'Monthly') ? $row['Salary'] : $row['Salary'];
                $salaryMonthOld = ($salaryType === 'Daily') ? ($salaryOld * 26) : $salaryOld;

                EmployeeFinalScore::updateOrCreate(
                    ['employee_no' => $employeeNo, 'rec_year' => $checkYear],
                    [
                        'import_id'         => $this->id,
                        'service_days'      => $row['SERVICE_DAYS'] ?? 0,
                        'attendance_sl'     => $row['SL'] ?? 0,
                        ...
                        'attendance_wwar'   => $row['WWAR'] ?? 0,
                        'attendance_vwar'   => $row['VWAR'] ?? 0,
                        'compliance_score'  => max($calcompliance, 1),
                        'attendance_score'  => ($row['SL'] ?? 0) + ($row['PL'] ?? 0) + ($row['LATE'] ?? 0) + ($row['ABS'] ?? 0),
                        'salary_type'       => $salaryType,
                        'salary_old'        => $salaryOld,
                        'bsalary_wage'      => $salaryOld,
            ...
            throw $e; // จะได้เห็น error ชัดๆ ตอน debug
        }

    }
    public function chunkSize(): int
    {
        return 500; // อ่าน 500 แถวต่อรอบ
    }

    public function batchSize(): int
    {
        return 100; // insert ทีละ 100 records
    }
}
